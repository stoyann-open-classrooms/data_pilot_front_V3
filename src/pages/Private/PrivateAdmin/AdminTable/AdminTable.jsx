...
import { createLine, getLinesForTable, updateLine, deleteLine, reset } from "../../../../features/line/lineslice";
import Modal from '../../../../components/shared/modal/Modal';
import PermissionTable from "../../../../components/table/PermissionTable/PermissionTable";

function AdminTable() {
  const { table, isLoading, isError, message } = useSelector((state) => state.table);
  ...
  const handleEditLineSubmit = (e) => {
    ...
    console.log("Dispatching updateLine with data:", { lineId: editLineData.id, lineData });
    dispatch(updateLine({ lineId: editLineData.id, lineData }))
      ...
      .catch((error) => {
        ...
        toast.error("Une erreur s'est produite lors de la modification de la ligne.");
      });
  };
    ...
    if (window.confirm("Êtes-vous sûr de vouloir supprimer cette ligne ?")) {
      dispatch(deleteLine(row.id))
        ...
        .catch((error) => {
          ...
          toast.error("Une erreur s'est produite lors de la suppression de la ligne.");
        });
    }
  };
  ...
  const handleNewLineSubmit = (e) => {
    ...
    dispatch(createLine(lineData))
      ...
      .catch((error) => {
        ...
        toast.error("Une erreur s'est produite lors de la création de la ligne.");
      });
  };
  ...
  <form onSubmit={handleEditLineSubmit}>
    {columns.map((column, colIndex) => (
      <div className="form-group" key={colIndex}>
        ...
        <input
          type={column === 'Date de début' || column === 'Date de fin' ? 'date' : 'text'}
          name={column}
          required
