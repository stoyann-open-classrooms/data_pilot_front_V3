...
import { getAuthorizations, updateAuthorization, deleteAuthorization } from "../../../../features/permission/permissionSlice"; // Importer les actions nécessaires
...
function AutorizedTableUserList() {
    const { authorizations, isLoading, isError, message } = useSelector((state) => state.authorization);
    ...
    const filteredAuthorizations = authorizations.data
        ? authorizations.data.filter((auto) => String(auto.tableau._id) === params.id)
        : [];

    const handleEdit = (id) => {
        const authorizationToEdit = authorizations.data.find(auth => auth._id === id);
    ...
    const handleUpdateAuthorization = async (e) => {
        e.preventDefault();
        try {
            await dispatch(updateAuthorization({
    ...
    if (isLoading || !authorizations.data) {
...
<div>
    {auto.read ? <FaCheck color="green" /> : <FaTimes color="red" />}
                    ...
                    <div>
                        <FaEdit className="icon-button" onClick={() => handleEdit(auto._id)} style={{ marginRight: '10px', cursor: 'pointer' }} />
                        <FaTrash className="icon-button delete" onClick={() => handleDelete(auto._id)} style={{ cursor: 'pointer' }} />
                    </div>
                </Ticket>
            ))}
            {isEditModalOpen && (
                    ...
                    <form onSubmit={handleUpdateAuthorization}>
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                onChange={(e) => setEditAuthorizationData(prevState => ({ ...prevState, read: e.target.checked }))} />
                        </div>
                        <div>
                            ...
                            <input 
                                ...
                                onChange={(e) => setEditAuthorizationData(prevState => ({ ...prevState, write: e.target.checked }))} />
                        </div>
